...
| High Phase D/E gate | Real legal evidence contains sensitive data or malicious files | Phase C requires a clean scanner result, private encrypted storage, owner/assigned-advocate authorization, short-lived URLs, access audit and legal holds | Select and validate the production scanner, approve evidence minimisation/retention/access policy, and complete cross-user/expiry/incident tests before real evidence |
| High Phase D/E gate | An operator or stale advocate identity issues legal work without current authority | Linked named advocate MFA, verification/authority snapshot, immutable review/quote/issue records, exact artifact hashes, and no reusable signature image | Verify enrolment and scope operationally, expose only a dedicated scoped advocate interface, and complete authenticated advocate UAT before publication |
| High Customer Beta gate | Public Flow endpoint is abused or a stale/cross-user form changes questionnaire facts | Global beta mode, encrypted bounded envelope, signed expiring order/product/schema token, owner check, row lock, strict field allowlist, sequence check, immutable final revision, request-rate bound and no commerce/legal-service entitlement | Complete Meta endpoint/public-key health check, real-device synthetic concurrency/replay tests and edge rate limiting before production beta enablement |
