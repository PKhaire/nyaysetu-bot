...
is the original baseline and `20260911_01` is the current schema head.
...
adds the private advocate-issued workflow foundation; `20260911_01` adds a
classification-specific golden-artifact map while retaining legacy
self-service PDF/DOCX approval evidence:
...
- `document_orders` is the aggregate root. It records a random public
  ...
  eligibility/current step/state; output classification; consent evidence;
  immutable confirmed-answer hash/revision; quoted price/currency; Razorpay
  link/payment evidence; preview/final availability windows; and terminal
  expiry/cancellation timestamps. It does not store a public object URL.
- `document_answer_revisions` stores immutable canonical JSON snapshots with a
  monotonically increasing revision and SHA-256 content hash. A payment and
  final artifact bind to one exact confirmed revision; edits create a new
  revision and invalidate the previous commercial/rendering path.
- `document_audit_events` stores privacy-minimised transition evidence and
  bounded metadata. Draft text, answer JSON, personal addresses, presigned
  links, credentials and generated bytes must never be written to it.
- `document_capacity_reservations` allocates one global India-business-day
  drafting slot to an order. `RESERVED` and `CONSUMED` rows count against the
  configured daily limit; only an unconsumed ineligible, cancelled or expired
  draft can transition to `RELEASED`. PostgreSQL transaction advisory locks
  serialize count-and-reserve operations for the business date.
- `document_template_approvals` is append-only authenticated release history
  for an exact product/template/questionnaire/renderer and aggregate/golden
  artifact hashes. Existing self-service approvals retain their dedicated
  PDF/DOCX fields; new output classes use a canonical typed artifact-hash map.
  The latest exact decision controls release; `REJECTED`,
  `REVOKED`, expired, incomplete or mismatched evidence fails closed.
- `document_artifacts` stores metadata only: order, kind, classification,
  exact answer/template/renderer hashes, content hash, byte size, private
  object key, creation/availability/deletion timestamps and lifecycle state.
...
presigned downloads. Unattached/unpaid drafts expire after the configured
seven-day window; final artifacts become unavailable and are deleted after the
...
Phase D registers the real cheque-notice package and its system-only intake
evaluation privately. The global allowlist still contains only the residential
agreement, so registration does not authorize customer visibility, real
evidence ingestion, payment, issuance or dispatch.
